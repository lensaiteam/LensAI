...
WAL) that is the moat. See [`CLAUDE.md`](./CLAUDE.md) for the architecture and the
...
a stray Node 20 that `npx`/`cmd` default to (plus a corrupt `node` shim in
`C:\Users\yash\node_modules\.bin`), which **segfaults** the addon. Until that's

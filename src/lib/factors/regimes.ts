...
const RULES: RegimeRule[] = [
  { key: "funding_regime", stream: "funding_rate", window: "365d", scope: "per_asset", labels: ["elevated", "neutral", "suppressed"] },
  { key: "oi_regime", stream: "open_interest", window: "365d", scope: "per_asset", labels: ["elevated", "neutral", "suppressed"] },
  { key: "dollar_regime", stream: "macro_dxy", window: "365d", scope: "market", marketAsset: "DXY", labels: ["strong", "neutral", "weak"] },
];
...
export function computeRegimes(db: DB, opts: ComputeRegimeOptions): RegimeRow[] {
  ...
  for (const rule of RULES) {
    const rows = derived.getPercentiles({ stream: rule.stream, window_id: rule.window, as_of: opts.asOf, slot: opts.slot });
    const relevant = rule.scope === "market" ? rows.filter((r) => r.asset === rule.marketAsset) : rows;
